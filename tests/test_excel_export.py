...
def test_rows_to_xlsx_splits_months_into_separate_tabs():
    rows = [
        {"name": "Alice", "date": "20-Jul", "hours": 9.0, "source": "Branch A / July"},
        {"name": "Alice", "date": "1-Aug", "hours": 9.5, "source": "Branch A / August"},
    ]
    data = excel_export.rows_to_xlsx(rows)
    wb = load_workbook(BytesIO(data))
    ...
    assert [c.value for c in wb["July"][2]] == ["Alice", "20-Jul", 9.0, "Branch A / July"]
    assert [c.value for c in wb["August"][2]] == ["Alice", "1-Aug", 9.5, "Branch A / August"]
